...
xscale('log')
yscale('linear')
axis([0.4,1051,0.,0.28])
xlabel(r"$y^+$")
ylabel(r"$\varepsilon$")# = \varepsilon_r + \varepsilon_t$")

plot(y_dns,-diss_dns,'-',color='k',label=r'$DNS$')
#plot(nomod.y,nomod.diss,'--',color='r',label=r'$No.mod.$')
plot(smag.y,-smag.diss,'--',color='g',label=r'$Std.Smag.$')
plot(wale.y,-wale.diss,'--',color='b',label=r'$Wale$')
plot(dyn0.y,-dyn0.diss,'-.',color='r',label=r'$Dyn.Smag.1$')
plot(dyn1.y,-dyn1.diss,'-.',color='b',label=r'$Dyn.Smag.2$')
#plot(dyn0b.y,dyn0b.diss,':',color='r',label=r'$Dyn.(def.)+<>$')
#plot(dyn1b.y,dyn1b.diss,':',color='b',label=r'$Dyn.(clip)+<>$')

legend(bbox_to_anchor=(1.4,0.9),numpoints=1)
