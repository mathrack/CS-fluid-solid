...
axis([0.4,1051,0.,11])
xlabel(r"$y^+$")
ylabel(r"$\overline{{{u'}_x}^2}$")#R_{xx}$")

plot(y_dns,rxx_dns,'-',color='k',label=r'$DNS$')
#plot(nomod.y,nomod.rxx,'--',color='r',label=r'$No.Mod.$')
plot(smag.y,smag.rxx,'--',color='g',label=r'$Std.Smag.$')
plot(wale.y,wale.rxx,'--',color='b',label=r'$Wale$')
plot(dyn0.y,dyn0.rxx,'-.',color='r',label=r'$Dyn.Smag.1$')
plot(dyn1.y,dyn1.rxx,'-.',color='b',label=r'$Dyn.Smag.2$')
#plot(dyn0b.y,dyn0b.rxx,':',color='r',label=r'$Dyn.(def.)+<>$')
#plot(dyn1b.y,dyn1b.rxx,':',color='b',label=r'$Dyn.(clip)+<>$')

legend(bbox_to_anchor=(1.4,0.9),numpoints=1)

savefig("rxx.png",bbox_inches='tight')
savefig("rxx.pdf",bbox_inches='tight')
