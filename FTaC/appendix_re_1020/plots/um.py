...
xscale('log')
yscale('linear')
axis([0.4,1051,0.35,25])
xlabel(r"$y^+$")
ylabel(r"$U_x^+$")

plot(y_dns,u_dns,'-',color='k',label=r'$DNS$')
plot(nomod.y,nomod.u,'--',color='r',label=r'$No.mod.$')
plot(smag.y,smag.u,'--',color='g',label=r'$Smag.$')
plot(wale.y,wale.u,'--',color='b',label=r'$Wale$')
plot(dyn0.y,dyn0.u,'-.',color='r',label=r'$Dyn.(def.)$')
plot(dyn1.y,dyn1.u,'-.',color='b',label=r'$Dyn.(clip)$')
plot(dyn0b.y,dyn0b.u,':',color='r',label=r'$Dyn.(def.)+<>$')
plot(dyn1b.y,dyn1b.u,':',color='b',label=r'$Dyn.(clip)+<>$')

legend(bbox_to_anchor=(1.6,0.9),numpoints=1)

savefig("um.png",bbox_inches='tight')
savefig("um.pdf",bbox_inches='tight')
