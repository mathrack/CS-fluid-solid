...
xscale('log')
yscale('linear')
axis([0.4,1051,-0.01,0.13])
xlabel(r"$y^+$")
ylabel(r"$C_s$")

#plot(y_dns,rxx_dns,'-',color='k',label=r'$DNS$')
#plot(nomod.y,nomod.mu,'--',color='r',label=r'$No.mod.$')
#plot(smag.y,smag.cs,'--',color='g',label=r'$Smag.$')
#plot(wale.y,wale.cs,'--',color='b',label=r'$Wale$')
#plot(dyn0.y,dyn0.cs,'-.',color='r',label=r'$Dyn.(def.)$')
#plot(dyn1.y,dyn1.cs,'-.',color='b',label=r'$Dyn.(clip)$')
plot(dyn0b.y,dyn0b.cs,':',color='r',label=r'$Dyn.(def.)+<>$')
plot(dyn1b.y,dyn1b.cs,':',color='b',label=r'$Dyn.(clip)+<>$')

legend(bbox_to_anchor=(1.6,0.9),numpoints=1)

savefig("cs.png",bbox_inches='tight')
savefig("cs.pdf",bbox_inches='tight')
