...
xscale('log')
yscale('linear')
axis([0.4,1051,-0.5,1.6])
xlabel(r"$y^+$")
ylabel(r"$\nu_t / \nu$")

#plot(y_dns,rxx_dns,'-',color='k',label=r'$DNS$')
#plot(nomod.y,nomod.mu,'--',color='r',label=r'$No.mod.$')
plot(smag.y,smag.mu,'--',color='g',label=r'$Smag.$')
plot(wale.y,wale.mu,'--',color='b',label=r'$Wale$')
plot(dyn0.y,dyn0.mu,'-.',color='r',label=r'$Dyn.(def.)$')
plot(dyn1.y,dyn1.mu,'-.',color='b',label=r'$Dyn.(clip)$')
plot(dyn0b.y,dyn0b.mu,':',color='r',label=r'$Dyn.(def.)+<>$')
plot(dyn1b.y,dyn1b.mu,':',color='b',label=r'$Dyn.(clip)+<>$')

legend(bbox_to_anchor=(1.6,0.9),numpoints=1)

savefig("nu.png",bbox_inches='tight')
savefig("nu.pdf",bbox_inches='tight')
