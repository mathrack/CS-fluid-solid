...
! call field_get_val_s(icrom, cpro_rom) ! (ro = constante = 1)
...
if (ntcabs.le.10000) then
  do iel = 1, ncel
!    crvimp(1, 1, iel) = - volume(iel)*cpro_rom(iel)*ckp
    crvimp(1, 2, iel) = volume(iel)*rot !cpro_rom(iel)*rot ! (ro = constante = 1)
    crvimp(2, 1, iel) = volume(iel)*rot !cpro_rom(iel)*rot ! (ro = constante = 1)
  enddo
endif

do iel = 1, ncel
  crvexp(1, iel) = volume(iel)*qdm!cpro_rom(iel)*qdm ! (ro = constante = 1)
enddo
