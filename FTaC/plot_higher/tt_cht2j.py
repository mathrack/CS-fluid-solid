...
for x in range(iscal, iscal+7):
     plot(insert(wale.scal[x].y,31,0),insert(wale.scal[x].tt,31,wale.ttfs[x-2]),'--',color='g')
plot(wale.scal[x].y[:31],wale.scal[x].tt[:31],'--',color='g',label=r'$G=0.2$')
...
savefig("tt_cht2j.png",bbox_inches='tight')
savefig("tt_cht2j.pdf",bbox_inches='tight')
