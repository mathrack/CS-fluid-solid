...
iscal = 9
for x in range(iscal, iscal+7):
     plot(append(wale.scal[x].y[:31],0),append(-wale.scal[x].diss[:31],wale.eps_fs[x-2]),'--',color='g')
     plot(insert(wale.scal[x].y[31:],0,0),insert(-wale.scal[x].diss[31:],0,wale.epf_fs[x-2]),'--',color='g')
plot(wale.scal[x].y[:31],-wale.scal[x].diss[:31],'--',color='g',label=r'$G=0.2$')
...
savefig("diss_cht2j.png",bbox_inches='tight')
savefig("diss_cht2j.pdf",bbox_inches='tight')
