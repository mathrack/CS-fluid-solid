...
if (ntcabs.eq.ntmabs) then
  ...
  ! Update initial analytical solution with time dependence
  if (.true.) then ! Euler implicite
    time_evol_m = 1d0/(1d0+num*dble(nm*nm)*pipi*ttcabs/rhom)
    time_evol_p = 1d0/(1d0+nup*dble(np*np)*pipi*ttcabs/rhop)
  else ! Analytical solution
    time_evol_m = exp(-num*dble(nm*nm)*pipi*ttcabs/rhom)
    time_evol_p = exp(-nup*dble(np*np)*pipi*ttcabs/rhop)
  endif
  do iel = 1, ncel
    x = xyzcen(1,iel)
    if (x .le. 0.d0) then
      cvar_exa(iel) = cvar_exa(iel) * time_evol_m
    else
      cvar_exa(iel) = cvar_exa(iel) * time_evol_p
    endif
  enddo
  ...
  do iel = 1, ncel
    x = xyzcen(1,iel)
    if (x .le. 0.d0) then
      grade(iel) = grade(iel) * time_evol_m
    else
      grade(iel) = grade(iel) * time_evol_p
    endif
  enddo
