...
integer       i, ifac, iel, nbfac, j
integer, dimension(:), pointer :: lsfac
...
do ifac = 1, nfabor
   itypfb(ifac) = iparoi
   do j=1,2
      icodcl(ifac, isca(j)) = 3
      rcodcl(ifac, isca(j), 3) = 0.
   enddo
enddo

! Bottom or top wall
call getfbr('x<-.9999999 or x>.9999999', nbfac, lsfac)
do i = 1, nbfac
   ifac = lsfac(i)
   iel = ifabor(ifac)
   itypfb(ifac) = iparoi
   do j = 1, 2
      icodcl(ifac, isca(j)) = 1
      rcodcl(ifac, isca(j), 1) = 1.
   enddo
enddo

! Thin wall
call getfbr('-.0000001<x<.0000001',nbfac,lsfac)
if (nbfac > 0) then
  do i = 1, nbfac
    ifac = lsfac(i)
    itypfb(ifac) = iparoi
...
! Other walls
call getfbr('y<.000001 or z<.000001 or y>.9999999 or z>.9999999', nbfac, lsfac)
do i = 1, nbfac
   ifac = lsfac(i)
   iel = ifabor(ifac)
   itypfb(ifac) = iparoi
