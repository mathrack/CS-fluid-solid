...
double precision, dimension(:), pointer :: cvar_diff, cvar_diff2
double precision, dimension(:), pointer :: cvar_rho, cvar_rho1, cvar_rho2
double precision x
...
!< [init]
iscal = 1
! --- Stop if the cvar_diffsivity is not variable
...
!call field_get_val_s(iprpfl(irom), cvar_rho)
!call field_get_key_int(ivarfl(isca(2)), kivisl, ifcvsl)
!call field_get_val_s(ifcvsl, cvar_diff2)
call field_get_key_int(ivarfl(isca(1)), kromsl, ifcvsl)
call field_get_val_s(ifcvsl, cvar_rho1)
call field_get_key_int(ivarfl(isca(2)), kromsl, ifcvsl)
call field_get_val_s(ifcvsl, cvar_rho2)
...
!   cvar_rho(iel) = 123.
   if(x.lt.0d0) then
!      cvar_diff(iel) = 0.!num
      cvar_rho1(iel) = 1.!rhom
   else
!      cvar_diff(iel) = 0.!nup
      cvar_rho1(iel) = 1.!rhop
   endif
!   cvar_diff2(iel) = cvar_diff(iel)
   cvar_rho2(iel) = cvar_rho1(iel)
   visten(1,iel) = 1.
   visten(2,iel) = 1.
   visten(3,iel) = 1.
   visten(4,iel) = 0.5d0
...
'@',/,                                                            &
'@    For scalar', i10,/,                                         &
'@      the cvar_diffsivity is uniform while',/,                      &
