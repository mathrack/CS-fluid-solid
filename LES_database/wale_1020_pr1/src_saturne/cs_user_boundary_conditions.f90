...
! Default boundary condition is:
!  - zero velocity
!  - homogeneous Neumann boundary condition for scalars
do ifac = 1, nfabor
  itypfb(ifac) = iparoi
  rcodcl(ifac,iu,1) = 0.0
  rcodcl(ifac,iv,1) = 0.0
  rcodcl(ifac,iw,1) = 0.0
  do ii = 1, nscal
    icodcl(ifac,isca(ii)) = 3 ! Neumann
    rcodcl(ifac,isca(ii),3) = 0. ! flux = 0
  enddo
enddo

! Check if some scalars have internal coupling
do ii = 1, nscal
  call field_get_key_struct_var_cal_opt(ivarfl(isca(ii)), vcopt)
  if (vcopt%icoupl .gt. 0) then
    this_scalar_is_coupled(ii) = .true.
  else
    this_scalar_is_coupled(ii) = .false.
  endif
enddo

call getfbr('plane[0,1,0,1,0.0001] or plane[0,1,0,-1,0.0001]', nlelt, lstelt)

do ilelt = 1, nlelt
  ifac = lstelt(ilelt)
  iel = ifabor(ifac)
  if (abs(xyzcen(2,iel)).lt.1.d0) then

    itypfb(ifac) = iparoi

    if (nscal.gt.0) then
      ii=1
      icodcl(ifac,isca(ii)) = 1 ! Dirichlet
      rcodcl(ifac,isca(ii),1) = 0.0

      if (nscal.gt.1) then
        ii=2
        icodcl(ifac,isca(ii)) = 3 ! Neumann
        rcodcl(ifac,isca(ii),3) = 1.d0*visls0(ii)

      endif ! nscal.gt.1
    endif ! nscal.gt.0
  endif ! abs(xyzcen(2,iel)).lt.1.d0 
enddo ! ilelt = 1, nlelt

call getfbr('plane[0,1,0,1.14706,0.0001] or plane[0,1,0,-1.14706,0.0001]', nlelt, lstelt)

do ii = 1, nscal
  if (this_scalar_is_coupled(ii)) then
    do ilelt = 1, nlelt
      ifac = lstelt(ilelt)
      icodcl(ifac,isca(ii)) = 3 ! Neumann for Conjug case at exterior solid wall
      rcodcl(ifac,isca(ii),3) = 1.d0*visls0(ii)
    enddo ! ilelt = 1, nlelt
  endif ! this_scalar_is_coupled(ii)
enddo ! ii = 1, nscal
