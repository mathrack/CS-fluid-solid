...
cs_user_mesh_modify(cs_mesh_t  *mesh)
{
  static int nx = 200-1;
  static int nz = 200-1;
  static double lx = 6.283;
  static double lz = 3.142;
  double dx = lx/(1.0*(nx+1));
  double dz = lz/(1.0*(nz+1));
  char criteria[60];

//  snprintf(criteria, sizeof(criteria), "x > %18f", 0.95*dx);
  snprintf(criteria, sizeof(criteria), "z > %18f", 0.95*dz);
...
//  cs_mesh_extrude_constant(mesh, false, nx, nx*dx, 1.0, n_selected_faces, selected_faces);
  cs_mesh_extrude_constant(mesh, false, nz, nz*dz, 1.0, n_selected_faces, selected_faces);
