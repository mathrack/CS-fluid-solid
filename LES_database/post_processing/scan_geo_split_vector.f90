...
program import_geo_split_tensor
  ...
  if (narg.le.1) then
    ...
    print *,''
    print *,'Program designed to split a vector written in the format Ensight Gold'
    print *,''
  ...
  print *,''
  print *,'Step 2 : splitting vectors'
  print *,''

  do iarg=2,narg
    call get_command_argument(iarg,argument)
    print *,'Splitting file : ',argument
    call split_vector(argument, nelt)
    print *,'Completed'
  enddo
